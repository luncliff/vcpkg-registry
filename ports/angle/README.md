...
- ✅ Null-backend build now links `EGL.dll` and `GLESv2.dll` on x64-windows (debug) using the editable install command below. Output shows only MSVC C4273 dll-linkage warnings from autogenerated GLES entry points; no unresolved symbols remain.
- ✅ Packages install to `packages/angle_x64-windows` with CMake config files emitted.
- ✅ D3D9 + D3D11 backends now link on x64-windows (debug) with autogen staged into the binary tree; only MSVC C4273 dll-linkage warnings remain from GLES autogen (same as Null backend).

### Implementation Plan (Phase 1: Windows + Null Backend)

- [x] **Step 1**: Expand `angle_common` to full `libangle_common_sources` ✅
- [x] **Step 2**: Add `angle_image_util` helper library ✅
- [x] **Step 3**: Add `angle_gpu_info_util` (Windows GPU info only) ✅
...
- ✅ `angle_common`, `angle_image_util`, `angle_gpu_info_util`
- ✅ `angle_translator` (GLSL/ESSL/HLSL backends)
- ✅ `libANGLE` core with Null backend
- ✅ Real `EGL` / `GLESv2` entry point sources compile (autogen + stubs) and link into DLLs
...
- ✅ `compression_utils_portable` replaced with a small system-zlib shim
- ❌ Frame capture implementation - disabled via `ANGLE_CAPTURE_ENABLED=0`
- 🚧 D3D9 + D3D11 backends - Windows desktop only (in progress, Phase 2)
- ❌ Vulkan/Metal/OpenGL backends - deferred to Phase 3+
- ❌ Abseil containers - optional, excluded initially
...
✅ **CMakeLists.txt Updated**: All D3D9/D3D11 sources, autogen generation, defines, and link flags added  
✅ **Autogen Inputs Staged**: `angle_format` helpers and DXGI/D3D11 JSON inputs copied to the binary tree before running generators  
✅ **Build Passed (x64-windows dbg)**: `vcpkg install --editable angle` now links `EGL.dll` / `GLESv2.dll` with D3D9 + D3D11 enabled (warnings: MSVC C4273 dll-linkage only)
...
- `gen_dxgi_format_table.py` → `dxgi_format_map_autogen.cpp`
- `gen_dxgi_support_tables.py` → `dxgi_support_table_autogen.cpp`
...
- `gen_blit11helper.py` → `Blit11Helper_autogen.inc` (included by `Blit11.cpp`)
- `gen_texture_format_table.py` → `texture_format_table_autogen.cpp`
...
- ✅ `angle_libangle` includes full D3D9 and D3D11 backend sources
- ✅ All autogen artifacts generated cleanly in binary tree
- ✅ `EGL.dll` / `GLESv2.dll` link without unresolved symbols
- ✅ D3D11 selected as primary backend on Windows with D3D9 as fallback
...
- ✅ `angle_common.lib`, `angle_image_util.lib`, `angle_gpu_info_util.lib`, `angle_translator.lib`, `libANGLE.lib`
- ❌ Link failure at `EGL.dll` on x64-windows (debug)
- ⚠️ `GLESv2.dll` status is not meaningful until `EGL.dll` links cleanly (the build stops at the first failing target)
